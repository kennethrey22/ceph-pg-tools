...
while IFS= read -r LINE; do
    ...
    # Check if the raw output is valid JSON
    if ! echo "$RAW_ATTRS_OUTPUT" | jq empty 2>/dev/null; then
        echo "[!] Invalid JSON output for attributes of OID: $OID"
        echo "    Raw output: $RAW_ATTRS_OUTPUT"
        echo "$OID: $RAW_ATTRS_OUTPUT" >> /tmp/invalid_attrs.log

        # Fallback: Save raw output to a file for manual inspection
        echo "$RAW_ATTRS_OUTPUT" > "${FILE_PREFIX}.attrs.raw"
        continue
    fi
